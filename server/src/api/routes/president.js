...
presidentRouter.post("/", async (req, res) => {
  ...
  return newPresident
    .save()
    .then((data) =>
      res.json({ message: "New president added Successfully!", _id: data._id })
    )
...
presidentRouter.delete("/:presidentId", async (req, res) => {
  President.findByIdAndDelete(req.params.presidentId)
    .then((data) =>
      res.json({ message: "President Deleted Successfully!", _id: data._id })
    )
    .catch((err) => res.status(500).json(err));
});
  ...
  President.findOne({ email })
    .then((data) => {
      if (data?.password === password) {
        ...
        return res.json({
          message: "Authenticated as a president successfully!",
          user: userData,
        });
      }
